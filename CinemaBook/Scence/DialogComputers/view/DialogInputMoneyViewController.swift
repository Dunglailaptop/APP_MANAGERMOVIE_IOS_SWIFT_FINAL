        ...
        if((sender as AnyObject).titleLabel?.text == "ĐỒNG Ý"){
            if (self.result) > maxMoney {
                ...
                if (self.result) != 0 && (self.result) < minMoney && isEnterZeroMoney {
                    JonAlert.show(message: String(format: "Số tiền tối thiểu phải bằng 0 hoặc lớn hơn %@", Utils.stringVietnameseMoneyFormatWithNumber(amount: minMoney)),andIcon: UIImage(named: "icon-warning"), duration: 2.5)
                    return
                } else if (self.result) < minMoney && !isEnterZeroMoney {
                    JonAlert.show(message: String(format: "Số tiền tối thiểu phải %@", Utils.stringVietnameseMoneyFormatWithNumber(amount: minMoney)),andIcon: UIImage(named: "icon-warning"), duration: 2.5)
                    return
                }
